...
import {Scorer,parseResultParams,resultParams,verdictOf,shareText,gateFailure,verdictLabel} from './web/score.js';
...
for(const lang of ['ja','zh-CN','en']){
 ...
 assert.ok(cardSVG(result,scorer,{lang:'ko'}).includes(`font-family="'Noto Sans KR', sans-serif"`)&&cardSVG(result,scorer,{lang:'ko'}).includes(`>${verdictLabel(result.verdict,'ko')}<`),'the Korean card names its verdict in Korean');
...
assert.equal(shareText({verdict:'female',display:28},'en'),'My voice on Koenami: Feminine voice (leaning feminine +28)');
assert.equal(shareText({verdict:'male',display:-3},'ko'),'내 목소리는 남성적인 목소리였습니다 (남성 쪽 −3)');
...
assert.equal(gateFailure({voiced_seconds:.6,formant_seconds:1,clipping_fraction:0},'en').need,'1 s or more');
