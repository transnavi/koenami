...
type QueueItem = { speaker: string; clips: ReviewClip[]; first: string; repeat?: string; previous?: { ratings: Record<string, number>; note: string }; missing?: string[] };
type Scale = { key: string; name: string; group: string; only?: string; decades?: boolean; ends?: string[] };
type Anchor = { scale: string; value: string; display?: string; end: string; audio: string };
type Draft = { clip?: string; ratings: Record<string, number>; chosen?: string[]; note?: string; active?: number; scope?: unknown };
type LogEntry = { display?: string; speaker?: string; flags?: string[]; ratings?: Record<string, number>; note?: string };
type Saved = { lang?: string; mode?: string; pass?: string; skipped?: Record<string, string[]>; speaker?: string; drafts?: Record<string, Draft>; ratings?: Record<string, number>; clip?: string; chosen?: string[]; scope?: unknown; note?: string; active?: number };
type State = { lang: string; mode: string; pass: string; session: string; drafts: Record<string, Draft>; queue: QueueItem[]; flags: Record<string, string>; at: number; clip: number; ratings: Record<string, number>; chosen: Set<string>; active: number; log: LogEntry[]; skipped: string[]; offered?: string[]; scales?: Scale[]; anchors?: Anchor[]; decades?: Record<string, string>; reviewed?: number };
...
export function mountReview() {
	defineKoeSelect();
	const state: State = { lang: 'ja', mode: 'new', pass: 'all', session: sessionId(), drafts: {}, queue: [], flags: {}, at: 0, clip: 0, ratings: {}, chosen: new Set(), active: 0, log: [], skipped: [] };
	function draftOf(): Draft { const item = current(); return { clip: item?.clips[state.clip]?.id, ratings: state.ratings, chosen: [...state.chosen], note: $<HTMLTextAreaElement>('note').value, active: state.active }; }
	...
	function recall(): Saved { try { const saved: Saved = JSON.parse(localStorage.getItem(STORAGE) as string) || {}; if (saved.ratings && saved.speaker && !saved.drafts) saved.drafts = { [saved.speaker]: { clip: saved.clip, ratings: saved.ratings, chosen: saved.chosen, scope: saved.scope, note: saved.note, active: saved.active } }; return saved; } catch { return {}; } }
	...
	async function load(saved: Saved = {}) {
		...
		state.skipped = (saved.skipped?.[state.lang + ':' + state.mode] || []).filter((sid) => (data.queue as QueueItem[]).some((q) => q.speaker === sid));
		state.queue = (data.queue as QueueItem[]).filter((q) => !state.skipped.includes(q.speaker)).concat(state.skipped.map((sid) => (data.queue as QueueItem[]).find((q) => q.speaker === sid)!));
		state.drafts = { ...state.drafts, ...(saved.drafts || {}) }; state.at = Math.max(0, state.queue.findIndex((q) => q.speaker === saved.speaker));
		for (const b of document.querySelectorAll<HTMLElement>('#mode button')) b.setAttribute('aria-pressed', String(b.dataset.mode === state.mode)); for (const b of document.querySelectorAll<HTMLElement>('#pass button')) b.setAttribute('aria-pressed', String(b.dataset.pass === state.pass));
		show();
	}
	function current(): QueueItem | undefined { return state.queue[state.at]; }
	...
	function show() {
		...
		const base = draft || (item.previous && { ratings: item.previous.ratings, chosen: [] as string[], note: item.previous.note, active: Math.max(0, scales().findIndex((s) => item.missing?.includes(s.key))) });
		state.clip = Math.max(0, item.clips.findIndex((c) => c.id === (draft?.clip || item.first))); state.ratings = { ...(base?.ratings || {}) }; state.chosen = new Set(base?.chosen || []); state.active = base?.active || 0; $<HTMLTextAreaElement>('note').value = base?.note || ''; $('note').blur(); $('status').textContent = '';
	...
	function renderJump() {
		...
		state.queue.forEach((q, i) => { const label = (q.clips[0].display || q.speaker).replace(/^[FM]\s*/, '№ '), text = q.clips.find((c) => c.id === q.first)?.text || ''; if (filter && !(label + ' ' + text).toLowerCase().includes(filter)) return;
			...
			const small = document.createElement('small'); small.textContent = text; b.append(name, mark, small); b.onclick = () => { go(i); $<HTMLDialogElement>('list-dialog').close(); }; box.append(b); });
		$('list-count').textContent = state.queue.length + '人';
	}
	function openList() { renderJump(); $<HTMLDialogElement>('list-dialog').showModal(); $('list-filter').focus(); requestAnimationFrame(() => $('list-items').querySelector('[aria-current=true]')?.scrollIntoView({ block: 'center' })); }
	function go(i: number) { if (i < 0 || i >= state.queue.length || i === state.at) return; stash(); state.at = i; show(); }
	...
	function step(d: number) { const item = current(); if (!item) return; state.clip = (state.clip + d + item.clips.length) % item.clips.length; renderClip(); remember(); play(); }
	function renderScales() {
		...
		scales().forEach((s, i) => {
			...
			const choices: [number, string][] = s.decades ? Object.entries(state.decades!).map(([v, label]) => [Number(v), label]) : Array.from({ length: 7 }, (_, v) => [v, String(v)]);
	...
	function key(d: number) { const s = scales()[state.active]; if (!s) return; if (s.decades) { const v = Object.keys(state.decades!).map(Number)[d - 1]; if (v === undefined) return; rate(v); } else rate(d); state.active = Math.min(scales().length - 1, state.active + 1); renderScales(); remember(); }
	function rate(v: number) { const s = scales()[state.active]; if (!s) return; if (state.ratings[s.key] === v) delete state.ratings[s.key]; else state.ratings[s.key] = v; renderScales(); remember(); }
	...
	function toggle(flag: string) { if (state.chosen.has(flag)) state.chosen.delete(flag); else state.chosen.add(flag); renderFlags(); remember(); }
	let inflight = false;
	async function save() {
		const item = current(); if (!item || inflight) return; const c = item.clips[state.clip];
		const body = { speaker: item.speaker, clip: c.id, display: c.display, language: state.lang, mode: item.repeat || state.mode, session: state.session, pass: state.pass, flags: [...state.chosen], ratings: state.chosen.size ? {} : state.ratings, note: $<HTMLTextAreaElement>('note').value };
	...
	document.addEventListener('keydown', (e) => {
		if ($<HTMLDialogElement>('list-dialog').open) { if (e.key === 'Escape') $<HTMLDialogElement>('list-dialog').close(); return; }
	...
	$('jump').onclick = openList; $('list-filter').oninput = renderJump; $('list-dialog').querySelector<HTMLElement>('[data-close]')!.onclick = () => $<HTMLDialogElement>('list-dialog').close(); $('prev-speaker').onclick = () => go(state.at - 1);
	...
	$('lang').onchange = () => { stash(); const previous = state.lang; state.lang = $<HTMLSelectElement>('lang').value; load(recall()).catch((e) => { state.lang = previous; $<HTMLSelectElement>('lang').value = previous; $('status').textContent = e.message; }); };
	for (const b of document.querySelectorAll<HTMLElement>('#pass button')) b.onclick = () => { state.pass = b.dataset.pass!; for (const o of document.querySelectorAll('#pass button')) o.setAttribute('aria-pressed', String(o === b)); state.active = 0; renderScales(); remember(); };
	for (const b of document.querySelectorAll<HTMLElement>('#mode button')) b.onclick = () => { if (state.mode === b.dataset.mode) return; stash(); const previous = state.mode; state.mode = b.dataset.mode!; load(recall()).catch((e) => { state.mode = previous; $('status').textContent = e.message; }); };
	(window as unknown as { reviewApp: State }).reviewApp = state;
	const saved = recall(); if (saved.mode === 'update') state.mode = 'update'; if (saved.pass) state.pass = saved.pass; if (saved.lang && [...$<HTMLSelectElement>('lang').options].some((o) => o.value === saved.lang)) { state.lang = saved.lang; $<HTMLSelectElement>('lang').value = saved.lang; }
