...
export const ICONS: Record<string, string> = {
	...
	misskey: '<path d="M3.5 19V7.2c0-1.3 1-2.2 2.2-2.2 1 0 1.6.5 2.1 1.4L12 12.5l4.2-6.1c.5-.9 1.1-1.4 2.1-1.4 1.2 0 2.2.9 2.2 2.2V19" stroke-width="2.8"/><path d="M12 12.5V19" stroke-width="2.8"/>',
...
export const icon = (name: string) => `<svg class="share-icon" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${ICONS[name]}</svg>`;
...
export function intents(url: string, text: string): Intent[] {
	return [
		...
		{ id: 'bluesky', label: 'Bluesky', icon: 'bluesky', href: `https://bsky.app/intent/compose?${new URLSearchParams({ text: `${text} #${HASHTAG}\n${url}` })}` },
...
export async function systemShare(result: ScoreResult, scorer: CardScorer, lang: string): Promise<boolean> {
