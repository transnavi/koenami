	...
	if (!indexPromise)
		indexPromise = fetch('/api/import-index/jvs')
			.then(async (response) => {
				...
				return new Map(
					((await response.json()) as { clips: ImportClip[] }).clips.map((c) => [c.id, c])
				);
			})
			.catch((error) => {
				indexPromise = null;
...
export async function loadImported<C extends ImportClip = ImportClip>(): Promise<
	(C & { localLibrary: true })[]
> {
	...
	return ids
		...
		.map((id) => ({ ...entries.get(id)!, localLibrary: true as const }));
}
...
function member(name: string): string | undefined {
	return name
		.replaceAll('\\', '/')
		.match(/(?:^|\/)(jvs\d{3}\/(?:parallel100|nonpara30)\/wav24kHz16bit\/[^/]+\.wav)$/)?.[1];
}

export async function importJVS(
	...
	try {
		if (files.length === 1 && /\.zip$/i.test(files[0].name)) {
			...
			for await (const entry of archive.getEntriesGenerator()) {
				...
				if (clip && !ids.has(clip.id) && entry.uncompressedSize === clip.bytes)
					...
					candidates.set(clip.id, {
						clip,
						read: () => (entry as FileEntry).getData(new BlobWriter('audio/wav'), { signal })
					});
			}
		} else {
			for (const file of files) {
				const clip = byMember.get(member(file.webkitRelativePath || file.name));
				if (clip && !ids.has(clip.id) && file.size === clip.bytes)
					candidates.set(clip.id, { clip, read: async () => file });
			}
		}
		if (!candidates.size) {
		...
		for (const { clip, read } of candidates.values()) {
			...
			const digest = [
				...
				.join('');
			if (digest !== clip.original_sha256)
				throw new Error('公式音声と一致しないファイルがあります。公式のZIPを選び直してください。');
			signal.throwIfAborted();
			ids.add(clip.id);
