...
function sessionId() {
	...
	try {
		s = JSON.parse(localStorage.getItem('koenami-review-session') as string);
	} catch {}
	if (!s || now - s.at > 40 * 60 * 1000)
		s = { id: Math.random().toString(36).slice(2, 10) + now.toString(36), at: now };
	s.at = now;
	try {
		localStorage.setItem('koenami-review-session', JSON.stringify(s));
	} catch {}
	return s.id;
}
export function mountPairs() {
	...
	function current(): Pair | undefined {
		return state.queue[state.at];
	}
	function remember() {
		const p = current();
		if (p)
			state.drafts[pairKey(p)] = {
		...
		try {
			localStorage.setItem(
				STORAGE,
				JSON.stringify({ lang: state.lang, key: p ? pairKey(p) : null, drafts: state.drafts })
			);
		} catch {}
	}
	function recall(): { key?: string | null; drafts?: Record<string, Draft> } {
		try {
	...
	let playing: Sd | null = null,
		loop = true;
	function mark() {
		for (const side of ['a', 'b'] as Sd[])
			$('side-' + side).setAttribute(
				'aria-pressed',
				String(playing === side && !players[side].paused)
			);
		$('loop').setAttribute('aria-pressed', String(loop));
	}
	for (const side of ['a', 'b'] as Sd[]) {
		const other = side === 'a' ? 'b' : 'a';
		players[side].onended = () => {
			if (loop) playSide(other);
			else {
				playing = null;
				mark();
			}
		};
		players[side].onplay = players[side].onpause = mark;
	}
	function playSide(side: Sd) {
		for (const s of ['a', 'b'] as Sd[]) if (s !== side) players[s].pause();
		playing = side;
		players[side].currentTime = 0;
		players[side].play().catch(() => {});
		mark();
	}
	function hold(side: Sd) {
		loop = false;
		playSide(side);
	}
	function restart() {
	...
	function togglePause() {
		const p = playing && players[playing];
		if (!p) return restart();
	...
	async function load(saved: { key?: string | null; drafts?: Record<string, Draft> } = {}) {
		const r = await fetch(
			'/api/pairs?lang=' + encodeURIComponent(state.lang) + '&session=' + state.session
		);
		...
		state.at = Math.max(
			0,
			state.queue.findIndex((p) => pairKey(p) === saved.key)
		);
		show();
	}
	function show() {
		...
		const draft = state.drafts[pairKey(p)] || ({} as Partial<Draft>);
		state.answers = { ...draft.answers };
		state.active = draft.active || 0;
		$<HTMLTextAreaElement>('note').value = draft.note || '';
		$('note').blur();
		$('status').textContent = '';
		for (const side of ['a', 'b'] as Sd[]) {
			$('text-' + side).textContent = p[side].text || '';
			$('meta-' + side).textContent = (p[side].duration || 0).toFixed(1) + ' 秒';
			players[side].src = p[side].audio;
		}
		renderQuestions();
		remember();
		if (!draft.answers) restart();
	}
	function renderQuestions() {
		...
		Object.entries(state.questions).forEach(([key, name], i) => {
			...
			(
				...
				kb.textContent = k;
				b.append(kb);
				b.setAttribute('aria-pressed', String(state.answers[key] === v));
				b.onclick = (e) => {
					e.stopPropagation();
					state.active = i;
					answer(v);
	...
	function answer(v: string) {
		const keys = Object.keys(state.questions),
			key = keys[state.active];
		if (!key) return;
		if (state.answers[key] === v) delete state.answers[key];
		else state.answers[key] = v;
		state.active = Math.min(keys.length - 1, state.active + 1);
		renderQuestions();
		remember();
	}
	let inflight = false;
	async function save() {
		...
		if (!Object.keys(state.answers).length) {
			$('status').textContent = '少なくとも1問に答えてください';
			return;
		}
		const body = {
			a: p.a.id,
		...
		try {
			...
			state.log.push({
				...(await r.json()),
				labels: [p.a.display || p.a.speaker, p.b.display || p.b.speaker]
			});
			state.judged++;
			delete state.drafts[pairKey(p)];
			state.at++;
			show();
		} catch (e) {
	...
	function renderLog() {
		...
		for (const r of state.log.slice(-8).reverse()) {
			...
			const labels = r.labels || [r.a, r.b];
			s.textContent = labels.map((l) => (l || '').replace(/^[FM]\s*/, '№ ')).join(' vs ');
			const parts = Object.entries(r.answers || {}).map(
				([k, v]) =>
					(state.questions[k] || k).replace(/^どちら(が|を)/, '') +
					' ' +
					(({ a: 'A', b: 'B', same: '同じ' } as Record<string, string>)[v] || v)
			);
			div.append(s, ' ' + parts.join(' · ') + (r.note ? ' · ' + r.note : ''));
			$('log').append(div);
		}
	}
	document.addEventListener('keydown', (e) => {
		...
		if (k === ' ') togglePause();
		else if (k.toLowerCase() === 'q') hold('a');
		else if (k.toLowerCase() === 'w') hold('b');
			...
			state.active = Math.min(Object.keys(state.questions).length - 1, state.active + 1);
			renderQuestions();
			remember();
		} else if (k === '1') answer('a');
		else if (k === '2') answer('same');
		else if (k === '3') answer('b');
		else if (k === 'Enter') void save();
		else if (k.toLowerCase() === 's') skip();
	});
	$('side-a').onclick = () => hold('a');
	$('side-b').onclick = () => hold('b');
	$('loop').onclick = restart;
	$('save').onclick = save;
	$('skip').onclick = skip;
