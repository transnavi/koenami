			...
			this.list.replaceChildren(
				...options.map((o) => {
					...
					const actions: Record<string, { label: string; path: string }> = {
						download: { label: 'ダウンロード', path: 'M10 3v10m-4-4 4 4 4-4M4 13v4h12v-4' },
						delete: { label: '削除', path: 'M3 5h14M8 5V3h4v2M5 5l1 12h8l1-12M8 8v6m4-6v6' }
					};
					for (const action of o.dataset.actions.split(',')) {
						...
						button.setAttribute('aria-label', o.textContent + 'を' + spec.label);
