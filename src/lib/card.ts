...
import { METRIC_KEYS, METRIC_LABELS, METRIC_UNITS, METRIC_DIGITS, VERDICTS, LEANINGS, formatScore, type ScoreResult, type MetricKey, ageText, plausibleAge } from './score';
...
export const CARD_WIDTH = 1200,
	CARD_HEIGHT = 630;
export type CardScorer = { bands?: Record<'female' | 'male', [number, number]>; metricBands?: Record<MetricKey, Record<'female' | 'male', [number, number]>>; cloud?: [number, number, string][] };
...
function slider(x: number, y: number, w: number, result: ScoreResult, bands: Bands) {
	const pos = (s: number) => x + w * clamp((s + 60) / 120, 0, 1);
	const band = (g: 'female' | 'male', color: string) =>
		bands?.[g] ? `<rect x="${pos(bands[g][0])}" y="${y - 7}" width="${Math.max(2, pos(bands[g][1]) - pos(bands[g][0]))}" height="14" rx="7" fill="${color}" opacity=".28"/>` : '';
...
function metricColumn(x: number, y: number, w: number, key: MetricKey, value: number, bands: Bands) {
	...
	const band = (g: 'female' | 'male', color: string) =>
		bands?.[g]?.every(finite) ? `<rect x="${pos(bands[g][0])}" y="${y + 58}" width="${Math.max(2, pos(bands[g][1]) - pos(bands[g][0]))}" height="10" rx="5" fill="${color}" opacity=".3"/>` : '';
...
function cloud(x: number, y: number, size: number, points: [number, number, string][], point: [number, number] | null) {
...
export function cardSVG(result: ScoreResult, scorer: CardScorer, { fonts = null, site = 'koe.transnavi.jp' }: { fonts?: Record<string, string> | null; site?: string } = {}) {
	const style = fonts
		? `<style>${Object.entries(fonts).map(([weight, uri]) => `@font-face{font-family:"Noto Sans JP";font-weight:${weight};src:url(${uri}) format("truetype")}`).join('')}</style>`
		: '';
