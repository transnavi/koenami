...
export function cardSVG(
	...
	}: { fonts?: Record<string, string> | null; site?: string } = {}
) {
	const style = fonts
		? `<style>${Object.entries(fonts)
				.map(
					([weight, uri]) =>
						`@font-face{font-family:"Noto Sans JP";font-weight:${weight};src:url(${uri}) format("truetype")}`
				)
				.join('')}</style>`
		: '';
	const accent =
	...
	const metrics = METRIC_KEYS.map((key, i) =>
		metricColumn(64 + i * 218, 470, 170, key, result.features[key], scorer.metricBands?.[key])
	).join('');
