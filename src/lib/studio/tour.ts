...
const icon = (id: string, cls = '') =>`<span class="tour-key ${cls}"><svg aria-hidden="true"><use href="#i-${id}"/></svg></span>`;
...
const phone = { get matches() { return (phoneQuery ??= matchMedia('(max-width:800px),(max-height:520px)')).matches; } };
const load = (): { step?: number; done?: boolean } => { try { return JSON.parse(localStorage.getItem(KEY) as string) || {}; } catch { return {}; } };
const save = (v: unknown) => { try { localStorage.setItem(KEY, JSON.stringify(v)); } catch {} };

let shade: HTMLDivElement, spot: HTMLDivElement, blockers: HTMLDivElement[], card: HTMLDialogElement, step = 0, raf = 0;
function build(){
 ...
 card.addEventListener('click', (e) => { const act = (e.target as Element).closest<HTMLElement>('[data-act]')?.dataset.act;if(act==='next')next();else if(act==='back')show(step-1);else if(act==='later')pause();else if(act==='skip')finish();});
 ...
 card.addEventListener('focusout', (e) => { if (card.open && e.relatedTarget && !card.contains(e.relatedTarget as Node)) card.querySelector<HTMLElement>('[data-act=next]')!.focus(); });
 document.body.append(shade,card);
}
const visible = (el: Element | null): el is HTMLElement => {if(!el||!el.checkVisibility())return false;const r=el.getBoundingClientRect();return r.width>0&&r.height>0;};
...
function show(n: number) {
 ...
 card.querySelector('#tour-title')!.textContent = s.title; card.querySelector('#tour-text')!.innerHTML = s.text.split('\n').map((t) => `<p>${t.replace(/\{(\w+)\}/g, (_, id: string) => CHIPS[id as keyof typeof CHIPS])}</p>`).join(''); card.querySelector('.tour-art')!.innerHTML = ART[s.art as keyof typeof ART];
 ...
 place(); card.querySelector<HTMLElement>('[data-act=next]')!.focus();
}
