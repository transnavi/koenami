...
describe('math', () => {
	it('finite accepts only finite numbers', () => {
		golden('math.finite', [0, -1.5, NaN, Infinity, -Infinity, '1', null, undefined, true].map((v) => finite(v)));
	});
	it('quantile: empty, single, sorted interpolation, unsorted input', () => {
		golden('math.quantile', {
			...
			unsorted: [0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99].map((p) => quantile([5, 1, 4, 2, 3, 9, 0.5], p)),
			untouched: (() => { const a = [3, 1, 2]; quantile(a, 0.5); return a; })()
		});
	});
	it('clamp', () => {
		golden('math.clamp', [[5, 0, 1], [-1, 0, 1], [0.5, 0, 1], [NaN, 0, 1], [2, 3, 1]].map(([x, a, b]) => clamp(x, a, b)));
	});
	it('AXES table', () => {
