...
function fakeDocument() {
			...
			calls.push({
				...
				sample: Array.from(
					img.data.slice(
						...
						((img.height >> 1) * img.width + (img.width >> 1)) * 4 + 8
					)
				),
	...
	const createElement = () =>
		(canvas = { width: 0, height: 0, getContext: () => ctx } as typeof canvas);
...
describe('DensityCloud', () => {
	...
	async function run(
		...
		const calls = await Promise.all(
			fake.calls.splice(0).map(async (c) => ({
				...(c as object),
				sha256: await (c as { sha256: Promise<string> }).sha256
			}))
		);
	...
	it('renders both groups at typical scale, light and dark', async () => {
		const light = await run(points(60, 3), {
			width: 640,
			height: 400,
			scale: 500,
			colors,
		...
		const dark = await run(points(60, 3), {
			width: 640,
			height: 400,
			scale: 500,
			colors,
	...
	it('reuses buffers when the size is unchanged and reallocates when it grows', async () => {
		...
		const a = await run(
			points(20, 4),
			{ width: 300, height: 200, scale: 100, colors, dark: false },
			cloud
		);
		const b = await run(
			points(20, 5),
		...
		const c = await run(
			points(20, 5),
			{ width: 900, height: 500, scale: 2000, colors, dark: false },
			cloud
		);
	...
	it('skips empty groups, out-of-range points and synthetic samples', async () => {
		...
		golden('cloud.sparse', {
			...
			synthetic: await run(
				points(12, 8).map((p) => ({ ...p, sample: { ...p.sample, synthetic: true } })),
				{ width: 320, height: 240, scale: 300, colors, dark: false }
			),
			badColour: await run(points(30, 9), {
				width: 320,
				height: 240,
				scale: 300,
