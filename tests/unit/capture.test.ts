...
const registry: Record<
	string,
	new () => { process(inputs: Float32Array[][]): boolean; port: Port }
> = {};
vi.stubGlobal(
	'AudioWorkletProcessor',
	class {
		port = { onmessage: null as unknown, postMessage: (..._a: unknown[]) => {} };
	}
);
vi.stubGlobal('registerProcessor', (name: string, cls: never) => {
	registry[name] = cls;
});
...
function make() {
	...
	p.port.postMessage = (data: unknown, transfer?: unknown[]) => {
		...
		posted.push({
			...
			head: Array.from(packet.slice(0, 3)),
			tail: Array.from(packet.slice(-2)),
...
describe('voice-capture worklet', () => {
	it('registers under its name', () => {
		golden('capture.names', Object.keys(registry));
	});
