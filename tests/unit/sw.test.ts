...
class FakeRequest {
	...
	constructor(url: string, init: Init = {}) { this.url = abs(url); this.method = init.method || 'GET'; this.mode = init.mode || 'cors'; }
}
type Req = FakeRequest | string;
const keyOf = (r: Req, ignoreSearch = false) => { const u = new URL(typeof r === 'string' ? abs(r) : r.url); if (ignoreSearch) u.search = ''; return u.href; };

const html = (body: string) => () => new Response(body, { headers: { 'content-type': 'text/html' } });
const indexHtml = html('<script src="/assets/app-abc.js"></script><link href=\'/assets/style-1.css\'>url(/assets/img.png)');
const responses: Record<string, () => Response> = {
	'/': indexHtml, '/ja/': indexHtml,
	'/guide.html': html('<p>guide</p>'), '/tutorial.html': html('<p>tutorial</p>'), '/method.html': html('<p>method</p>'),
	'/site.webmanifest': () => new Response('{}', { headers: { 'content-type': 'application/manifest+json' } }),
	'/favicon.svg': () => new Response('<svg/>', { headers: { 'content-type': 'image/svg+xml' } }),
	'/icon-192.png': () => new Response(null),
	'/assets/app-abc.js': () => new Response('app', { headers: { 'content-type': 'text/javascript' } }),
	'/assets/old-zzz.js': () => new Response('old', { headers: { 'content-type': 'text/javascript' } }),
...
class FakeCache {
	...
	phantom: string[] = [];
	async keys() { return [...this.store.keys(), ...this.phantom].map((u) => new FakeRequest(u)); }
	...
	async put(r: Req, res: Response) { this.store.set(keyOf(r), res); }
	async delete(r: Req) { return this.store.delete(keyOf(r)); }
	async addAll(urls: string[]) { for (const u of urls) this.store.set(abs(u), await fetchStub(new FakeRequest(u))); }
}
...
type Handler = (event: { request?: FakeRequest; respondWith?: (p: Promise<Response>) => void; waitUntil: (p: Promise<unknown>) => void }) => void;
const handlers: Record<string, Handler> = {};
const skipWaiting = vi.fn(async () => {});
const claim = vi.fn(async () => {});
vi.stubGlobal('addEventListener', (type: string, handler: Handler) => { handlers[type] = handler; });
...
const request = async (url: string, init: Init = {}) => {
	...
	handlers.fetch({ request: new FakeRequest(url, init), respondWith: (p) => { response = p; }, waitUntil: (p) => waits.push(p) });
	const outcome = response ? await (response as Promise<Response>).then((r) => String(r.status), (e: Error) => `error: ${e.message}`) : 'passthrough';
...
describe('service worker', () => {
	it('caches the shell, hashed assets and pages, serves them offline, and prunes what no page references', async () => {
		...
		const requests: [string, Init?][] = [
			['/api/analyze', { method: 'POST' }], ['https://elsewhere.test/x'], ['/api/catalog'], ['/samples/a.wav'],
			['/assets/app-abc.js'], ['/assets/app-abc.js'], ['/assets/old-zzz.js'], ['/assets/missing.js'],
			['/ja/?utm=1', { mode: 'navigate' }], ['/guide.html', { mode: 'navigate' }], ['/site.webmanifest'], ['/robots.txt']
		];
		for (const [url, init] of requests) {
			if (url === '/guide.html') caches.get('koenami-v1')!.phantom.push(abs('/ghost.html'));
			log.push([init?.method || 'GET', url, init?.mode || 'cors', await request(url, init), cached(), fetchStub.mock.calls.length]);
		}
		online = false;
		for (const [url, init] of [['/en/', { mode: 'navigate' }], ['/nope.html', { mode: 'navigate' }], ['/site.webmanifest?v=2'], ['/assets/app-abc.js'], ['/assets/never.js']] as [string, Init?][]) {
