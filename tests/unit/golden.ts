...
if (record && (process.env.KOENAMI_TREE || 'old') !== 'old') throw new Error('RECORD=1 is only valid with KOENAMI_TREE=old');
...
export function canonical(value: unknown): unknown {
	...
	if (typeof value === 'number') return Number.isFinite(value) && !Object.is(value, -0) ? value : { $number: Object.is(value, -0) ? '-0' : String(value) };
	...
	if (ArrayBuffer.isView(value)) return { $typed: value.constructor.name, values: Array.from(value as unknown as ArrayLike<number>) };
...
export function golden(name: string, actual: unknown) {
	...
	if (!existsSync(file)) throw new Error(`missing golden ${name}; run with RECORD=1 against the old tree`);
