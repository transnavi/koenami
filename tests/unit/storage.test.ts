...
async function dump() {
	...
	return new Promise<Record<string, unknown>>((resolve, reject) => {
		...
		values.onsuccess = () => resolve(Object.fromEntries((keys.result as string[]).map((k, i) => [k, values.result[i]])));
		values.onerror = () => reject(values.error);
	});
}
const snap = (n: number) => ({ pcm: { $typed: 'Float32Array', values: [n, n + 1] }, measurement: { f0: 100 + n }, range: null });
const meta = (id: string, n: number) => ({ id, name: `take ${n}`, at: 1700000000000 + n, takeId: id });
...
describe('TakeStore', () => {
	...
	it('opens once, reads missing keys as undefined, writes pairs', async () => {
		const db1 = await TakeStore.open(), db2 = await TakeStore.open();
		expect(db1).toBe(db2);
		const before = await TakeStore.read();
		await TakeStore.write({ current: { takeId: 'a' }, previous: null });
		await TakeStore.writeEntries([[['x'], 'jvs-index'], [{ k: 1 }, 'custom']]);
		golden('storage.basic', { before, takes: await TakeStore.read(), jvs: await TakeStore.read('jvs-index'), all: await dump() });
	});
	...
	it('saveRecording / finishRecording / deleteRecording keep index and pair consistent', async () => {
		...
		const ranged = await TakeStore.saveRecording({ ...snap(4), range: [0, 1], measurement: { f0: 4 } }, meta('r3', 4));
		...
		golden('storage.recordings', { s1, s2, s1again, f1, fMissing, ranged, f3, afterFinish, d2, dMissing, d1, final: await dump() });
	});
	...
	it('deleteRecording without a saved pair or index', async () => {
		...
		golden('storage.delete-no-pair', { unindexed, result: await TakeStore.deleteRecording('solo'), final: await dump() });
	});
	...
	it('serializes writes and recovers after a failed operation', async () => {
		...
		const custom = TakeStore.recordingTransaction('q', () => ({ snapshot: snap(9), metadata: meta('q', 9) }));
	...
	it('surfaces transaction failures with and without an error object', async () => {
		...
		IDBDatabase.prototype.transaction = function () {
			const tx = { error, objectStore: () => ({ put: () => {}, delete: () => {}, get: () => ({}) }), oncomplete: null as unknown, onabort: null as unknown, onerror: null as unknown };
			setTimeout(() => (tx.onerror as () => void)());
			return tx as never;
		};
		results.push(await TakeStore.write({ current: null, previous: null }).catch((e) => e.message));
		...
		results.push(await TakeStore.recordingTransaction('r1', () => null).catch((e) => e.message));
		IDBDatabase.prototype.transaction = function () {
			const tx = { objectStore: () => ({ get: () => { const r = { error: new Error('read failed'), onerror: null as unknown }; setTimeout(() => (r.onerror as () => void)()); return r; } }) };
			return tx as never;
		};
	...
	it('rejects when the database cannot open', async () => {
		globalThis.indexedDB = { open: () => { const r: Record<string, unknown> = {}; setTimeout(() => { r.error = new Error('blocked'); (r.onerror as () => void)(); }); return r; } } as never;
