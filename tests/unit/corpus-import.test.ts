...
const clips = await Promise.all(members.map(async ([spk, set, name, size], i) => {
	const member = `jvs_ver1/${spk}/${set}/wav24kHz16bit/${name}.wav`;
	const bytes = wav(i + 1, size);
	files.set(member, bytes);
	return { id: `${spk}-${set}-${name}`, speaker: spk, name: spk.toUpperCase(), group: i % 2 ? 'female' : 'male', member, bytes: size, original_sha256: await sha256(bytes) };
}));
...
for (let i = 0; i < 12; i++) {
	...
	files.set(member, bytes);
	clips.push({ id: `jvs009-parallel100-EXTRA_${String(i).padStart(3, '0')}`, speaker: 'jvs009', name: 'JVS009', group: 'female', member, bytes: bytes.length, original_sha256: await sha256(bytes) });
}
...
function folder(entries: Iterable<[string, Uint8Array<ArrayBuffer>]>) {
	return [...entries].map(([path, bytes]) => Object.assign(new File([bytes], path.split('/').pop()!), { webkitRelativePath: path }));
}
...
async function run(input: File[], signal = new AbortController().signal) {
	...
	try {
		...
		return { error: String((error as Error).message ?? error), progress };
	}
}
async function stored() {
	const ids = (await TakeStore.read('jvs-index')) || [];
	return { ids, audio: await Promise.all(ids.map(async (id: string) => [id, await sha256(await ((await TakeStore.read('jvs-audio:' + id)) as Blob).arrayBuffer())])) };
}
...
describe('JVS import', () => {
	...
	it('nothing imported yet', async () => {
		golden('corpus.empty', { imported: await loadImported(), audio: await importedAudio('missing').catch((e) => e.message) });
	});
	...
	it('imports the whole zip, then reports zero additions on a repeat', async () => {
		...
		golden('corpus.zip', { first, again, stored: await stored(), loaded: loaded.map((c: { id: string; localLibrary: boolean }) => [c.id, c.localLibrary]), audio: await sha256(await (await importedAudio(clips[0].id)).arrayBuffer()) });
	});

	it('imports an extracted folder, with backslash paths and unknown files', async () => {
		const subset = [...files].slice(0, 3);
		const input = [...folder(subset.map(([p, b]) => [p.replaceAll('/', '\\'), b] as [string, Uint8Array<ArrayBuffer>])), new File([wav(9, 10)], 'notes.txt')];
	...
	it('rejects archives without matching members, size mismatches and digest mismatches', async () => {
		...
		class Rewritten extends File { reads = 0; get size() { return super.size + (this.reads++ ? 1 : 0); } }
		const rewritten = Object.assign(new Rewritten([files.get(clips[2].member)!], 'BASIC5000_0001.wav'), { webkitRelativePath: clips[2].member });
		golden('corpus.mismatch', { size: await run([wrongSize]), digest: await run([wrongDigest]), unrelated: await run([unrelated]), rewritten: await run([rewritten]), stored: await stored() });
	});
	...
	it('stops at the abort signal and keeps completed files', async () => {
		...
		vi.spyOn(crypto.subtle, 'digest').mockImplementation((alg, data) => { if (++calls === 3) controller.abort(); return original(alg, data); });
	...
	it('quota: absent estimate passes, insufficient quota refuses, index errors propagate and retry', async () => {
		...
		vi.stubGlobal('navigator', { storage: { estimate: async () => ({ quota: 1000, usage: 500 }) } });
		const tight = await run([await zip([...files].slice(1, 3))]);
		globalThis.indexedDB = new IDBFactory(); TakeStore.db = null;
		...
		const failed = await fresh([await zip(files)], () => {}, new AbortController().signal).catch((e: Error) => e.message);
		fetchImpl = async () => new Response(JSON.stringify({ clips }), { status: 200 });
		const recovered = await fresh([await zip([...files].slice(3, 4))], () => {}, new AbortController().signal);
