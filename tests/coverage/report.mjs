...
const sourcePath = (filePath) => {
	const clean = filePath.replace(/^127\.0\.0\.1[-:]\d+\//, '').replace(/(\.[cm]?js|\.ts|\.svelte)[^/]*$/, '$1').replace(/^.*?tests\/old-tree\//, '').replace(/^.*?(src\/lib\/)/, '$1');
...
const entryFilter = (entry) => {
	...
	const u = new URL(url);
	return u.hostname === '127.0.0.1' && !u.pathname.startsWith('/@') && !u.pathname.includes('/node_modules/') && /\.(js|ts|svelte)$/.test(u.pathname);
};
const options = (name, outputDir) => ({
	name, outputDir, entryFilter, sourcePath,
	reports: [['json', { file: 'coverage-final.json' }], ['console-summary']],
	sourceFilter: (path) => !path.includes('node_modules'),
	clean: false, cleanCache: true
});

let generated = 0;
const unitRaw = join(root, 'coverage/unit/raw');
if (existsSync(unitRaw)) {
	await MCR({ ...options('koenami unit coverage', join(root, 'coverage/unit')), inputDir: [unitRaw] }).generate();
	generated++;
}
...
if (existsSync(e2eRaw)) {
	...
	for (const name of readdirSync(e2eRaw)) await mcr.add(JSON.parse(readFileSync(join(e2eRaw, name), 'utf8')).result);
	await mcr.generate();
	generated++;
}
if (!generated) { console.error('no raw coverage; run test:unit:coverage and/or test:e2e first'); process.exit(1); }
