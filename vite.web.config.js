 ...
 return {
  ...
  configureServer(server) {
   server.middlewares.use((req, res, next) => {
    ...
    if (req.url === '/r' || req.url.startsWith('/r?')) { req.url = '/result.html' + req.url.slice(2); return next(); }
  ...
  generateBundle(options, bundle) {
   for (const [name, path] of Object.entries(PAGES)) {
    ...
    if (!asset || asset.type !== 'asset') throw new Error(`koenami-language-pages: ${name} is not in the bundle`);
    const template = String(asset.source);
    asset.source = renderPage(template, 'ja', path('ja'));
    for (const lang of LANGUAGES) {
     if (lang === 'ja' && name !== 'index.html') continue;
     this.emitFile({ type: 'asset', fileName: `${lang}/${name}`, source: renderPage(template, lang, path(lang)) });
    }
   }
   for (const lang of LANGUAGES) if (lang !== 'ja') this.emitFile({ type: 'asset', fileName: `${lang}/site.webmanifest`, source: renderManifest(manifest(), lang) });
  }
 };
}
