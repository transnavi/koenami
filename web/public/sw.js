...
const SHELL=['/','/zh-CN/','/en/','/ko/','/guide.html','/tutorial.html','/method.html','/site.webmanifest','/zh-CN/site.webmanifest','/en/site.webmanifest','/ko/site.webmanifest','/language.js','/favicon.svg','/icon-192.png'];
...
// Each shell file is fetched on its own, so one that fails to load keeps the rest cached.
addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>Promise.all(SHELL.map(path=>c.add(path).catch(()=>{})))).then(()=>skipWaiting()));});
