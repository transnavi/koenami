...
let noticeTimer,abTimer=null,audioContext=null,playbackNodes=null,recordContext=null,stream=null,worklet=null,analyser=null,chunks=[],samples=0,liveBusy=false,liveTimer=null,liveGeneration=0,blobURL=null,flushResolve=null,recordSnapshot=null,liveController=null,recordCamera=null,monitorGain=null,importedRefURL=null;
function notify(text,error=false){$(error?'live-alert':'live-status').textContent=text;clearTimeout(noticeTimer);$('notice').textContent=text;$('notice').classList.toggle('error',error);$('notice').hidden=false;noticeTimer=setTimeout(()=>$('notice').hidden=true,error?9000:4500);}
async function api(url,options){const r=await fetch(url,options);if(!r.ok)throw new Error((await r.text()).slice(0,200)||`Request failed (${r.status})`);return r.json();}
function setTheme(value){try{localStorage.setItem('voice-theme',value);}catch{}document.documentElement.dataset.theme=value==='system'?(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):value;$('theme-select').value=value;icon($('theme-button'),document.documentElement.dataset.theme==='dark'?'sun':'moon');map.invalidate();signal.dirty=true;}
$('theme-button').onclick=()=>setTheme(document.documentElement.dataset.theme==='dark'?'light':'dark');$('theme-select').onchange=e=>setTheme(e.target.value);
try{$('theme-select').value=localStorage.getItem('voice-theme')||'system';}catch{}icon($('theme-button'),document.documentElement.dataset.theme==='dark'?'sun':'moon');
for(const name of ['settings','info'])$(name+'-button').onclick=()=>{$(name+'-dialog').showModal();};
...
function speakerName(c){return c.synthetic?(c.name||c.speaker):nameOf(c);}
function clipRow(clip){
 const b=document.createElement('button');b.className='sample-row';b.dataset.id=clip.id;b.dataset.group=clip.group;b.setAttribute('aria-pressed',String(clip.id===state.selected?.id));
 ...
 b.title=clip.text;b.onclick=()=>selectSample(clip,true);b.disabled=state.recording||state.loadingLanguage;
 const row=document.createElement('div');row.className='sample-item';const star=document.createElement('button');star.className='favorite';star.textContent=favorites.has(clip.id)?'★':'☆';star.setAttribute('aria-pressed',String(favorites.has(clip.id)));star.setAttribute('aria-label',`${clip.text||nameOf(clip)} ${favorites.has(clip.id)?'をお気に入りから外す':'をお気に入りに追加'}`);star.onclick=()=>toggleFavorite(clip.id);row.append(b,star);return row;
}
function renderLibrary(reset=false){
 const focused=document.activeElement?.dataset?.id,scroll=$('sample-scroll').scrollTop;if(reset){state.limit=30;$('sample-scroll').scrollTop=0;}
 const clips=filtered(),groups=new Map();for(const clip of clips){const key=speakerKey(clip);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(clip);}
 $('library-count').textContent=String(groups.size);$('sample-list').replaceChildren();
 let index=0;for(const [key,items] of groups){if(index++>=state.limit)break;
  const folder=document.createElement('details');folder.className='speaker-folder';folder.dataset.speaker=key;folder.open=openSpeakers.has(key);
  const heading=document.createElement('summary'),favs=items.filter(c=>favorites.has(c.id)).length;
  ...
  const list=document.createElement('div');list.className='speaker-clips';
  const populate=()=>{list.replaceChildren();const limit=speakerLimits.get(key)||30;for(const c of items.slice(0,limit))list.append(clipRow(c));if(items.length>limit){const more=document.createElement('button');more.className='speaker-more';more.textContent='もっと見る';more.onclick=()=>{speakerLimits.set(key,limit+30);populate();};list.append(more);}};
  folder.addEventListener('toggle',()=>{if(!folder.isConnected)return;if(folder.open){openSpeakers.add(key);populate();}else openSpeakers.delete(key);});if(folder.open)populate();
  folder.append(heading,list);$('sample-list').append(folder);
 }
 $('load-more').hidden=groups.size<=state.limit;if(!reset)$('sample-scroll').scrollTop=scroll;
 if(focused)$('sample-list').querySelector(`[data-id="${focused}"]`)?.focus({preventScroll:true});updateMap();
}
$('load-more').onclick=()=>{state.limit+=60;renderLibrary();};
for(const id of ['sort','teacher','teacher-pitch','teacher-resonance','teacher-weight'])$(id).onchange=()=>renderLibrary(true);$('library-group').onchange=()=>{$('sort').value=$('library-group').value==='female'?'high':$('library-group').value==='male'?'low':'name';renderLibrary(true);};$('search').oninput=()=>renderLibrary(true);for(const id of ['show-female','show-male'])$(id).onchange=updateMap;
...
async function selectSample(clip,play=false){if(!clip||state.recording||state.loadingLanguage)return;cancelAB();reference.pause();const previousGroup=referenceGroup();state.selected=clip;document.documentElement.style.setProperty('--reference',clip.group==='male'?'var(--sky)':clip.group==='female'?'var(--pink)':'var(--other-reference)');signal.images=new WeakMap();signal.dirty=true;if(previousGroup!==referenceGroup())buildFit();openSpeakers.add(speakerKey(clip));const token=++state.detailToken;state.rangeToken.ref++;state.wordToken.ref++;state.ranges.ref=null;state.words.ref=null;state.ref=clip;state.refFull=null;state.refPCM=clip.pcm||null;signal.set('ref',null);let localPCM=null;if(clip.localLibrary){try{const file=await importedAudio(clip.id);if(token!==state.detailToken)return;if(importedRefURL)URL.revokeObjectURL(importedRefURL);importedRefURL=URL.createObjectURL(file);reference.src=importedRefURL;localPCM=await decode(file);if(token!==state.detailToken)return;state.refPCM=localPCM;}catch(e){notify(e.message,true);return;}}else reference.src=clip.audio;$('selected-name').textContent=nameOf(clip);updateFavorite();$('selected-meta').textContent=`${fmt(clip.features.f0)} Hz · ${fmt(clip.features.delta_f)} ΔF${clip.synthetic?' · 合成音声':''}`;$('selected-text').textContent=clip.text;$('selected-text').lang=state.lang;$('source-link').hidden=!clip.source;if(clip.source)$('source-link').href=clip.source;$('play-reference').disabled=false;renderLibrary();updateIndicators();renderWords();if(play)playSide('ref').catch(e=>notify(e.message,true));try{const detail=clip.detail||(localPCM?await api('/api/analyze',{method:'POST',body:localPCM}):await api('/api/detail/'+encodeURIComponent(clip.id)));if(token!==state.detailToken)return;state.ref=state.refFull=detail;signal.set('ref',detail);if(!state.ownFull)setSignalSource('ref');updateMap();updateIndicators();if(signal.source==='ref')updateRangeLabel();}catch(e){if(token===state.detailToken)notify('見本の解析：'+e.message,true);}}
function controls(){const near=$('sort').querySelector('option[value=near]');near.disabled=!AcousticSpace.raw(activeFeatures('own')).every(finite);if(near.disabled&&$('sort').value==='near')$('sort').value='name';const busy=state.busy||state.recording||state.loadingLanguage;for(const id of ['upload','reference-upload','add-reference','language','play-mine','compare-ab','reference-seek','words-button','range-reset'])$(id).disabled=busy;$('play-mine').disabled=busy||!state.ownFull;$('play-reference').disabled=busy||!state.selected;$('share-button').disabled=busy||!shareResult();$('share-button').title=state.scorer?.available?'判定を共有':'この言語の見本では判定を計算できません';
 $('record').disabled=state.busy||state.loadingLanguage||(state.recording&&state.captureMode==='live');$('record').setAttribute('aria-pressed',String(state.recording&&state.captureMode==='record'));$('record').setAttribute('aria-label',state.recording?'録音を停止':'新しく録音');icon($('record'),state.recording&&state.captureMode==='record'?'stop':'mic');$('record').title=state.recording&&state.captureMode==='record'?'録音を停止（R）':'録音（R）';
 $('live-mode').disabled=state.busy||state.loadingLanguage||(state.recording&&state.captureMode!=='live');$('live-mode').setAttribute('aria-checked',String(state.recording&&state.captureMode==='live'));const isLive=state.recording&&state.captureMode==='live';$('live-mode').setAttribute('aria-label',isLive?'リアルタイム測定を停止':'リアルタイム測定を開始');$('live-mode').title=isLive?'リアルタイム測定を停止（Esc）':'マイクの声をリアルタイムに表示';$('live-mode-label').textContent=isLive?'測定中':'リアルタイム';$('live-time').hidden=!isLive;$('loopback').disabled=!state.recording||state.busy;
 renderTakeMenu();$('state').textContent=state.recording?(state.captureMode==='live'?'測定中':'録音中'):state.busy?'準備中':state.analyzing.has(state.ownTakeId)?'解析中':'';$('state').hidden=!$('state').textContent;}
...
async function toggle(side){cancelAB();const el=side==='own'?player:reference;if(el.paused)await playSide(side);else el.pause();}
$('play-mine').onclick=()=>toggle('own').catch(e=>notify(e.message,true));$('play-reference').onclick=()=>toggle('ref').catch(e=>notify(e.message,true));$('normalize').onchange=()=>{if(audioContext)audioReady();};
for(const [side,el,id] of [['own',player,'play-mine'],['ref',reference,'play-reference']]){el.addEventListener('play',()=>{icon($(id),'pause');$(id).setAttribute('aria-label',side==='own'?'自分の声を一時停止':'見本を一時停止');map.invalidate();});el.addEventListener('pause',()=>{icon($(id),'play');$(id).setAttribute('aria-label',side==='own'?'自分の声を再生':'見本を再生');map.invalidate();signal.dirty=true;});el.addEventListener('error',()=>{if(el.src)notify('再生できませんでした。別の音声を選んでください。',true);});el.addEventListener('timeupdate',()=>{const d=el.duration;if(side==='ref'){if(finite(d))$('reference-seek').value=el.currentTime/d*100;$('reference-time').textContent=clock(el.currentTime);}else if(!state.recording)$('timer').textContent=state.ownFull?clock(el.currentTime)+' / '+clock(state.ownFull.duration):'0:00';const r=state.ranges[side];if(r&&el.currentTime>=r[1])el.pause();});}
function seek(side,time){if(state.recording)return;cancelAB();const el=side==='own'?player:reference;el.currentTime=clamp(time,0,finite(el.duration)?el.duration:time);map.invalidate();signal.dirty=true;}
$('reference-seek').oninput=e=>seek('ref',Number(e.target.value)/100*(reference.duration||0));
$('compare-ab').onclick=async()=>{
 if(abTimer){cancelAB();player.pause();reference.pause();return;}if(!state.selected||!state.ownFull)return;
 try{await audioReady();const refStart=state.ranges.ref?.[0]||0,ownStart=state.ranges.own?.[0]||0;
 const seconds=Math.min(12,(state.ranges.ref?.[1]||state.refFull?.duration||state.selected.duration)-refStart,(state.ranges.own?.[1]||state.ownFull.duration)-ownStart);
 if(!finite(seconds)||seconds<=0)throw new Error('両方の音声が読み込まれるまでお待ちください。');
 $('compare-ab').setAttribute('aria-pressed','true');await playSide('ref',true);let phase='ref';
 abTimer=setInterval(()=>{if(phase==='ref'&&(reference.paused||reference.currentTime-refStart>=seconds)){reference.pause();phase='starting';playSide('own',true).then(()=>phase='own').catch(e=>{cancelAB();notify(e.message,true);});}else if(phase==='own'&&(player.paused||player.currentTime-ownStart>=seconds)){player.pause();cancelAB();}},30);
 }catch(e){cancelAB();notify(e.message,true);}
};
for(const b of document.querySelectorAll('[data-dimension]'))b.onclick=()=>{map.dimension=Number(b.dataset.dimension);map.zoom=1;map.pan=[0,0];map.autoFit=map.dimension===3;map.fitDirty=true;$('auto-rotate').hidden=map.dimension!==3;map.projection=map.dimension===2?'contrast':'variance';for(const el of document.querySelectorAll('[data-dimension]'))el.setAttribute('aria-pressed',String(el===b));syncProjection();map.invalidate();updateMap();};
for(const b of document.querySelectorAll('[data-projection]'))b.onclick=()=>setProjection(b.dataset.projection);
function setProjection(name){if(!name||name===map.projection)return;map.projection=name;map.zoom=1;map.pan=[0,0];map.center=[.5,.5];syncProjection();map.invalidate();updateMap();}
function syncProjection(){const available=map.space?.projections||{};if(!available[map.projection])map.projection='variance';const controls=$('projection-controls');controls.hidden=map.dimension!==2;controls.querySelector('[data-projection=contrast]').disabled=!available.contrast;for(const el of document.querySelectorAll('[data-projection]'))el.setAttribute('aria-pressed',String(el.dataset.projection===map.projection));$('space-label').title=map.projection==='contrast'?'5つの指標から計算。横軸は女性的な声と男性的な声が最も離れる方向です。':'5つの指標から計算した主成分空間';}$('zoom-in').onclick=()=>map.zoomBy(1.2);$('zoom-out').onclick=()=>map.zoomBy(1/1.2);$('reset-view').onclick=()=>map.reset();$('find-me').onclick=()=>{const v=map.vector(activeFeatures('own'));if(v&&map.dimension===2){map.center=v.slice(0,2);map.invalidate();}else if(map.dimension===3){map.fitScope='voices';map.autoFit=true;map.fitDirty=true;map.invalidate();}};
function changeSpeed(e){const rate=Number(e.target.value);$('speed-reset').textContent=rate.toFixed(2).replace(/0+$/,'').replace(/\.$/,'')+'×';for(const el of [player,reference]){el.defaultPlaybackRate=rate;el.playbackRate=rate;el.preservesPitch=true;}try{localStorage.setItem('voice-speed',String(rate));}catch{}};
try{const speed=localStorage.getItem('voice-speed');if(speed&&Number(speed)>=.5&&Number(speed)<=1.5)$('playback-speed').value=speed;}catch{}$('playback-speed').oninput=changeSpeed;$('speed-reset').onclick=()=>{$('playback-speed').value='1';changeSpeed({target:$('playback-speed')});};changeSpeed({target:$('playback-speed')});
function syncSignalSegment(){$('signal-own').setAttribute('aria-pressed',String(!signal.overlay&&signal.source==='own'));$('signal-both').setAttribute('aria-pressed',String(signal.overlay));$('signal-ref').setAttribute('aria-pressed',String(!signal.overlay&&signal.source==='ref'));$('signal-both').title=signal.overlay?'各範囲の長さを0〜100%にそろえます。単語の位置は一致しません。':'長さをそろえて重ねて表示します';}function setSignalSource(side){signal.source=side;syncSignalSegment();signal.dirty=true;renderWords();updateRangeLabel();}function setSignalOverlay(on){signal.overlay=on;syncSignalSegment();signal.dirty=true;renderWords();updateRangeLabel();}$('signal-own').onclick=()=>{setSignalOverlay(false);setSignalSource('own');};$('signal-ref').onclick=()=>{setSignalOverlay(false);setSignalSource('ref');};$('signal-both').onclick=()=>setSignalOverlay(true);$('signal-view').onchange=e=>{signal.mode=e.target.value;signal.dirty=true;};
function updateRangeLabel(){const side=signal.source,r=state.ranges[side],d=(side==='own'?state.ownFull:state.refFull)?.duration||0;$('range-reset').hidden=!r;$('range-label').textContent=r?`${r[0].toFixed(2)}–${r[1].toFixed(2)}s`:clock(d);}
async function selectRange(side,range){if(state.recording)return;const full=side==='own'?state.ownFull:state.refFull;if(!full)return;const token=++state.rangeToken[side];state.ranges[side]=range;signal.setRange(side,range);updateRangeLabel();renderWords();if(!range){if(side==='own')state.own=full;else state.ref=full;updateMap();updateIndicators();if(sessionReady){persistTakes();saveView();}return;}seek(side,range[0]);updateMap();const id=side==='own'?state.ownId:state.selected?.id;try{let detail;if(id&&!id.startsWith('custom-')&&!(side==='ref'&&state.selected?.localLibrary))detail=await api(`/api/detail/${encodeURIComponent(id)}?start=${range[0]}&end=${range[1]}`);else{const pcm=side==='own'?state.ownPCM:state.refPCM;if(!pcm)return;detail=await api('/api/analyze',{method:'POST',body:pcm.slice(Math.round(range[0]*16000),Math.round(range[1]*16000))});detail.offset=range[0];}if(token!==state.rangeToken[side])return;if(side==='own')state.own=detail;else state.ref=detail;signal.setRange(side,range,detail);updateMap();updateIndicators();}catch(e){if(token===state.rangeToken[side])notify(e.message,true);}}
$('range-reset').onclick=()=>selectRange(signal.source,null);
function setLiveShapeWindow(value){map.liveShapeSeconds=clamp(Math.round(Number(value)||5),1,30);$('live-shape-window').value=String(map.liveShapeSeconds);$('live-shape-duration').textContent=map.liveShapeSeconds+' 秒';$('live-shape-window').setAttribute('aria-valuetext',map.liveShapeSeconds+' 秒');map.invalidate();}
$('live-shape-window').oninput=e=>{setLiveShapeWindow(e.target.value);saveView();};
function renderWords(){const side=signal.source,entries=state.words[side]?.words||[];$('word-list').replaceChildren();const range=state.ranges[side];for(const w of entries){const b=document.createElement('button');b.textContent=w.text;b.title=`${w.start.toFixed(2)}–${w.end.toFixed(2)}s · 推定位置`;b.className=range&&w.start>=range[0]-.01&&w.end<=range[1]+.01?'active':'';b.onclick=()=>{const duration=(side==='own'?state.ownFull:state.refFull)?.duration||0;const end=Math.min(duration,Math.max(w.end+.03,.25)),start=Math.max(0,Math.min(w.start-.03,end-.25));selectRange(side,[start,end]);};$('word-list').append(b);} }
$('words-button').onclick=async()=>{const side=signal.source,id=side==='own'?state.ownId:state.selected?.id;if(state.words[side]){renderWords();return;}const token=++state.wordToken[side];$('words-button').disabled=true;$('words-button').textContent='…';try{let result;const lang=side==='own'?state.ownLanguage:(state.lang==='lab'?'en':state.lang);if(id&&!id.startsWith('custom-')&&!(side==='ref'&&state.selected?.localLibrary))result=await api(`/api/words/${encodeURIComponent(id)}?lang=${lang}`);else{const pcm=side==='own'?state.ownPCM:state.refPCM;if(!pcm)throw new Error('先に音声を読み込んでください。');result=await api('/api/words?lang='+lang,{method:'POST',body:pcm});}if(token===state.wordToken[side]){state.words[side]=result;renderWords();}}catch(e){notify(e.message,true);}finally{$('words-button').disabled=false;$('words-button').textContent='単語';}};
async function decode(blob){if(blob.size>150*1024*1024)throw new Error('150 MB未満の音声を選んでください。');const ctx=new AudioContext();try{const decoded=await ctx.decodeAudioData(await blob.arrayBuffer());if(decoded.duration>900||decoded.duration<.25)throw new Error('0.25秒〜15分の音声を選んでください。');const off=new OfflineAudioContext(1,Math.ceil(decoded.duration*16000),16000),source=off.createBufferSource();source.buffer=decoded;source.connect(off.destination);source.start();return (await off.startRendering()).getChannelData(0).slice();}finally{await ctx.close();}}
function wav(pcm,rate=16000){const b=new ArrayBuffer(44+pcm.length*2),v=new DataView(b),str=(at,s)=>{for(let i=0;i<s.length;i++)v.setUint8(at+i,s.charCodeAt(i));};str(0,'RIFF');v.setUint32(4,36+pcm.length*2,true);str(8,'WAVE');str(12,'fmt ');v.setUint32(16,16,true);v.setUint16(20,1,true);v.setUint16(22,1,true);v.setUint32(24,rate,true);v.setUint32(28,rate*2,true);v.setUint16(32,2,true);v.setUint16(34,16,true);str(36,'data');v.setUint32(40,pcm.length*2,true);for(let i=0;i<pcm.length;i++)v.setInt16(44+i*2,Math.round(clamp(pcm[i],-1,1)*32767),true);return new Blob([b],{type:'audio/wav'});}
...
function qualityMessage(reason){return {'No reliable voiced speech. Check the microphone and speak normally.':'声を検出できませんでした。マイクの入力を確認してください。','Speak for a little longer.':'もう少し長く話してください。','Unstable resonance estimate.':'響きを安定して測定できませんでした。'}[reason]||reason||'';}
function setOwn(detail,name,id,pcm,url,remember=true){if(remember){const previous=state.captureMode?recordSnapshot:snapshotOwn();if(previous?.detail&&(previous.pcm||previous.url))state.previousTake=previous;state.ownTakeId=null;}state.ownToken++;state.rangeToken.own++;state.wordToken.own++;state.own=state.ownFull=detail;state.ownName=name;state.ownLanguage=id==='baseline'?'ja':(state.lang==='lab'?'en':state.lang);state.ownId=id;state.ownPCM=pcm;state.words.own=null;state.ranges.own=null;signal.set('own',detail);if(remember)setSignalSource('own');state.liveClock=null;signal.live=false;if(blobURL){URL.revokeObjectURL(blobURL);blobURL=null;}if(pcm){blobURL=URL.createObjectURL(wav(pcm));player.src=blobURL;}else player.src=url;$('timer').textContent=clock(detail.duration);updateIndicators();updateMap();renderWords();updateRangeLabel();const bad=qualityMessage(detail.reason);$('quality-state').textContent=bad;$('quality-state').hidden=!bad;controls();if(remember&&!recordSnapshot)persistTakes();}
async function importAudio(file,side){if(!file||state.busy||state.recording)return;cancelAB();player.pause();reference.pause();stopReplay();state.busy=true;controls();try{const pcm=await decode(file);if(pcm.length/16000>(state.capabilities?.maxSeconds||900))throw new Error(`${(state.capabilities?.maxSeconds||900)/60}分以内の音声を選んでください。`);const detail=await api('/api/analyze',{method:'POST',body:pcm});if(side==='own'){setOwn(detail,file.name,null,pcm);await saveTake();}else{const c={id:'custom-'+Date.now(),name:file.name,speaker:'custom-'+Date.now(),group:'custom',language:state.lang,audio:URL.createObjectURL(wav(pcm)),text:file.name,features:detail.features,duration:detail.duration,plotted:finite(detail.features.delta_f),detail,pcm};state.custom.push(c);$('library-group').value='custom';await selectSample(c,false);}notify('音声を読み込みました。');}catch(e){notify(e.message,true);}finally{state.busy=false;controls();$('upload').value='';$('reference-upload').value='';}}
$('upload').onchange=e=>importAudio(e.target.files[0],'own');$('reference-upload').onchange=e=>importAudio(e.target.files[0],'ref');function mergeChunks(limit=Infinity){const count=Math.min(chunks.reduce((n,c)=>n+c.length,0),limit),out=new Float32Array(count);let position=count;for(let i=chunks.length-1;i>=0&&position>0;i--){const n=Math.min(chunks[i].length,position);out.set(chunks[i].subarray(chunks[i].length-n),position-n);position-=n;}return out;}
async function resample(pcm,rate){if(rate===16000)return pcm;const ctx=new OfflineAudioContext(1,Math.ceil(pcm.length/rate*16000),16000),buffer=ctx.createBuffer(1,pcm.length,rate);buffer.copyToChannel(pcm,0);const source=ctx.createBufferSource();source.buffer=buffer;source.connect(ctx.destination);source.start();return (await ctx.startRendering()).getChannelData(0).slice();}
async function liveMeasure(){if(liveBusy||!state.recording||!recordContext||samples<recordContext.sampleRate*.5)return;liveBusy=true;const generation=liveGeneration,rate=recordContext.sampleRate,end=samples/rate,window=Number($('live-window').value),raw=mergeChunks(Math.floor(rate*window)),offset=end-raw.length/rate;try{const pcm=await resample(raw,rate),m=await api('/api/analyze?live=1',{method:'POST',body:pcm,signal:(liveController=new AbortController()).signal});if(generation!==liveGeneration||!state.recording)return;const rows=m.track.map(p=>({...p,t:p.t+offset}));const replaceAt=Math.max(offset+.12,end-window+.12);state.liveTrack=state.liveTrack.filter(p=>p.t<replaceAt&&(state.captureMode!=='live'||p.t>end-90)).concat(rows.filter(p=>p.t>=replaceAt));state.ownFull={...m,track:state.liveTrack,duration:end};state.own=m;state.ranges.own=null;state.liveClock={end:rows.at(-1)?.t||end,at:performance.now(),start:Math.max(0,end-.65)};signal.data.own=m;signal.ranges.own=[0,m.duration];signal.focus.own=null;signal.live=true;signal.dirty=true;map.own=state.ownFull;map.ownFeatures=m.features;map.ownRange=null;map.invalidate();$('quality-state').textContent=m.active?'':'音声を待っています…';$('quality-state').hidden=m.active;updateIndicators();}catch(e){if(generation===liveGeneration&&e.name!=='AbortError')notify(e.message,true);}finally{if(generation===liveGeneration)liveBusy=false;}}
async function startRecording(mode='record'){if(state.busy||state.recording||state.loadingLanguage)return;recordSnapshot=snapshotOwn();state.captureMode=mode;persistTakes();state.rangeToken.own++;state.wordToken.own++;state.busy=true;controls();cancelAB();player.pause();reference.pause();stopReplay();try{stream=await navigator.mediaDevices.getUserMedia({audio:{channelCount:1,echoCancellation:false,noiseSuppression:false,autoGainControl:false}});recordContext=new AudioContext({latencyHint:'interactive'});await recordContext.audioWorklet.addModule(new URL('./capture.js',import.meta.url));const source=recordContext.createMediaStreamSource(stream);worklet=new AudioWorkletNode(recordContext,'voice-capture');analyser=recordContext.createAnalyser();analyser.fftSize=2048;const mute=recordContext.createGain();mute.gain.value=0;source.connect(worklet);source.connect(analyser);monitorGain=recordContext.createGain();monitorGain.gain.value=0;source.connect(monitorGain);monitorGain.connect(recordContext.destination);worklet.connect(mute);mute.connect(recordContext.destination);chunks=[];samples=0;state.liveTrack=[];signal.source='own';setSignalSource('own');state.liveClock=null;state.recording=true;recordCamera={autoFit:map.autoFit,autoRotate:map.autoRotate,navigationVersion:map.navigationVersion};map.autoFit=false;map.autoRotate=false;$('auto-rotate').setAttribute('aria-pressed','false');map.live=mode==='live';state.busy=false;liveGeneration++;liveBusy=false;worklet.port.onmessage=({data})=>{if(data==='flushed'){flushResolve?.();flushResolve=null;return;}if(data instanceof Float32Array){chunks.push(data);samples+=data.length;if(state.captureMode==='live'){let retained=chunks.reduce((n,c)=>n+c.length,0);while(chunks.length>1&&retained-chunks[0].length>recordContext.sampleRate*12.2)retained-=chunks.shift().length;}else if(samples>=recordContext.sampleRate*(state.capabilities?.maxSeconds||900))stopRecording();}};await recordContext.resume();liveTimer=setInterval(liveMeasure,480);controls();}catch(e){await releaseMic();state.busy=false;state.recording=false;recordSnapshot=null;state.captureMode=null;controls();notify(e.name==='NotAllowedError'?'ブラウザーのマイク設定で、このページからの使用を許可してください。':e.message,true);}}
async function releaseMic(){if(monitorGain){monitorGain.disconnect();monitorGain=null;}$('loopback').setAttribute('aria-pressed','false');$('loopback').setAttribute('aria-label','自分の声を聴く');if(worklet){worklet.port.onmessage=null;worklet.port.close();}liveController?.abort();liveController=null;clearInterval(liveTimer);liveTimer=null;for(const t of stream?.getTracks()||[])t.stop();stream=null;if(recordContext){try{await recordContext.close();}catch{}}recordContext=null;worklet=null;analyser=null;}
function snapshotOwn(){if(!state.ownFull)return null;return {takeId:state.ownTakeId,detail:state.ownFull,measurement:state.own,range:state.ranges.own,words:state.words.own,pcm:state.ownPCM,name:state.ownName,id:state.ownId,language:state.ownLanguage,url:state.ownId==='baseline'?'/data/baseline.wav':null};}
function applySnapshot(t){if(!t?.detail)return;setOwn(t.detail,t.name,t.id,t.pcm,t.url,false);state.ownTakeId=t.takeId;state.ownLanguage=t.language;state.own=t.measurement||t.detail;state.ranges.own=t.range;state.words.own=t.words;signal.setRange('own',t.range,t.range?state.own:null);signal.live=false;state.liveClock=null;map.live=false;updateMap();updateIndicators();renderWords();updateRangeLabel();controls();}
...
async function analyzeTake(take){
 ...
 try{
  ...
  const updated=await TakeStore.finishRecording(id,detail);if(!updated)return;state.takes=updated.index;
 ...
 }catch(e){if(state.takes.some(t=>t.id===id))notify('録音を残しました。録音のメニューから再解析できます。',true);}
...
function reportHTML(){const f=activeFeatures('own'),r=activeFeatures('ref'),comparison=map.space?.comparison(f,r,map.dimension,map.projection),fit=fitValue(f),refs=referenceStats();const rows=METRICS.map(m=>{const vals=refs.map(c=>c.features[m.key]).filter(finite);return `<tr><td>${m.label} · ${m.unit}</td><td>${fmt(f[m.key],m.n)}</td><td>${fmt(r[m.key],m.n)}</td><td>${fmt(quantile(vals,.1),m.n)}〜${fmt(quantile(vals,.9),m.n)}</td></tr>`;});
...
async function init(){requestAnimationFrame(animate);controls();const catalog=await api('/api/catalog');state.capabilities=catalog.capabilities||{};$('words-button').hidden=state.capabilities.words===false;$('language').replaceChildren(...catalog.languages.map(l=>new Option(l.label,l.id)));
 const view=readView();setLiveShapeWindow(view?.liveShapeSeconds);const requested=location.pathname.split('/')[1]||view?.lang||'ja',lang=catalog.languages.some(l=>l.id===requested)?requested:'ja';
 const [saved,storedRefs,index]=await Promise.all([TakeStore.read().catch(()=>null),TakeStore.read('references').catch(()=>[]),TakeStore.read('recording-index').catch(()=>[])]);
 state.imported=await loadImported().catch(e=>{notify(e.message,true);return [];});state.takes=index||[];state.custom=(storedRefs||[]).filter(c=>c.pcm&&favorites.has(c.id)).map(c=>({...c,audio:URL.createObjectURL(wav(c.pcm))}));state.previousTake=saved?.previous?.pcm?saved.previous:null;
 ...
 if(view&&view.lang===lang){
  $('library-group').value=view.group||'female';$('sort').value=view.sort||'high';$('search').value=view.search||'';openSpeakers.clear();for(const key of view.openSpeakers||[])openSpeakers.add(key);renderLibrary(true);
  const clip=state.clips.concat(state.custom).find(c=>c.id===view.reference);if(clip)await selectSample(clip,false);
  if(view.referenceRange)await selectRange('ref',view.referenceRange);
  map.dimension=view.dimension===2?2:3;map.projection=view.dimension===2&&view.projection!=='variance'?'contrast':'variance';for(const b of document.querySelectorAll('[data-dimension]'))b.setAttribute('aria-pressed',String(Number(b.dataset.dimension)===map.dimension));$('auto-rotate').hidden=map.dimension!==3;
  for(const key of ['yaw','tilt','zoom'])if(finite(view[key]))map[key]=view[key];for(const key of ['camera','center','pan'])if(Array.isArray(view[key])&&view[key].every(finite))map[key]=view[key];map.autoFit=false;map.autoRotate=!!view.autoRotate;$('auto-rotate').setAttribute('aria-pressed',String(map.autoRotate));
  signal.mode=view.signal||'pitch';$('signal-view').value=signal.mode;signal.overlay=view.overlay!==false;setSignalSource(view.signalSource==='ref'?'ref':'own');map.invalidate();signal.dirty=true;syncProjection();
 }
 controls();sessionReady=true;window.voiceApp={state,map,signal,fitValue,selectRange,selectSample,changeLanguage,controls,TakeStore,snapshotOwn,captureDebug:()=>({bufferSeconds:chunks.reduce((n,c)=>n+c.length,0)/(recordContext?.sampleRate||1),monitoring:!!monitorGain&&monitorGain.gain.value>0,hasSnapshot:recordSnapshot!==null})};localize();
}



let profileTheme='';
function drawProfile(own,ref){
 const canvas=$('profile-canvas');if(!canvas)return;const w=canvas.clientWidth,h=canvas.clientHeight;if(!w||!h)return;
 const dpr=Math.min(devicePixelRatio||1,2);canvas.width=w*dpr;canvas.height=h*dpr;const c=canvas.getContext('2d');c.scale(dpr,dpr);const css=getComputedStyle(document.documentElement),color=k=>css.getPropertyValue(k).trim();const cx=w/2,cy=h/2+3,r=Math.min(w/2-28,h/2-23);
 const refs=state.representatives,limits=METRICS.map(m=>{const values=refs.map(p=>p.features[m.key]).filter(finite);return [quantile(values,.01),quantile(values,.99)];});
 const point=(i,ratio)=>[cx+Math.sin(i*Math.PI*2/5)*r*ratio,cy-Math.cos(i*Math.PI*2/5)*r*ratio];
 c.strokeStyle=color('--line');c.lineWidth=1;for(const scale of [.33,.66,1]){c.beginPath();for(let i=0;i<5;i++){const p=point(i,scale);i?c.lineTo(...p):c.moveTo(...p);}c.closePath();c.stroke();}
 c.font='10px system-ui';c.textAlign='center';c.fillStyle=color('--muted');for(let i=0;i<5;i++){const p=point(i,1),label=point(i,1.27);c.beginPath();c.moveTo(cx,cy);c.lineTo(...p);c.stroke();c.fillText(METRICS[i].label,label[0],label[1]+3);}
 for(const [f,key,dash] of [[ref,'--reference',[]],[own,'--self',[4,3]]]){if(!METRICS.every(m=>finite(f[m.key])))continue;const pts=METRICS.map((m,i)=>point(i,.12+.88*clamp((f[m.key]-limits[i][0])/(limits[i][1]-limits[i][0]||1),0,1)));c.beginPath();pts.forEach((p,i)=>i?c.lineTo(...p):c.moveTo(...p));c.closePath();c.strokeStyle=color(key);c.lineWidth=2;c.setLineDash(dash);c.stroke();c.setLineDash([]);c.fillStyle=color(key);c.globalAlpha=.06;c.fill();c.globalAlpha=1;for(const p of pts){c.beginPath();c.arc(...p,2.5,0,Math.PI*2);c.fill();}}
}
new ResizeObserver(()=>drawProfile(activeFeatures('own'),activeFeatures('ref'))).observe($('profile-canvas'));

function updateFavorite(){const yes=favorites.has(state.selected?.id);$('favorite-selected').textContent=yes?'★':'☆';$('favorite-selected').setAttribute('aria-pressed',String(yes));$('favorite-selected').setAttribute('aria-label',yes?'お気に入りから外す':'お気に入りに追加');}
function toggleFavorite(id){if(!id)return;if(favorites.has(id))favorites.delete(id);else favorites.add(id);try{localStorage.setItem('voice-favorites',JSON.stringify([...favorites]));}catch{notify('お気に入りを保存できませんでした。',true);}TakeStore.write(state.custom.filter(c=>favorites.has(c.id)).map(({audio,...c})=>c),'references').catch(()=>notify('見本の音声を保存できませんでした。',true));updateFavorite();renderLibrary();}
$('favorite-selected').onclick=()=>toggleFavorite(state.selected?.id);

$('auto-rotate').setAttribute('aria-pressed',String(map.autoRotate));$('auto-rotate').onclick=()=>{map.autoRotate=!map.autoRotate;$('auto-rotate').setAttribute('aria-pressed',String(map.autoRotate));map.invalidate();};
...
function renderTakeMenu(){const select=$('take-select');if(!select)return;const current=state.recording?recordSnapshot:snapshotOwn(),seen=new Set();takeChoices=[];
 ...
 select.replaceChildren();takeChoices.forEach((t,i)=>{const option=new Option(t.name,String(i));option.dataset.detail=clock(t.detail?.duration||t.duration);option.dataset.actions='play,rename,download,delete';option.dataset.key=replayKeyOf(t);const peaks=t.pcm?wavePeaks(t.pcm):t.peaks;if(peaks)option.dataset.peaks=JSON.stringify(peaks);if(state.recording||!(t.takeId||t.storedId))option.dataset.disabledActions='delete'+(state.recording?',play':'');select.add(option);});
...
async function deleteTake(take){
 ...
 const current=id===state.ownTakeId;state.busy=true;stopReplay();controls();
 try{
...
$('take-select').addEventListener('optionaction',async e=>{
 ...
 if(e.detail.action==='play')try{
  ...
  replayTake(source,replayKeyOf(chosen));
...
/* Row-level replay in the recording history; the waveform is the play control.
   Rows are addressed by take identity, because positional values shift between renders. */
let replayAudio=null,replayKey=null;
const replayKeyOf=take=>take?.storedId||take?.takeId||'mem:'+(take?.pcm?.length||0);
function stopReplay(){const key=replayKey;if(replayAudio){URL.revokeObjectURL(replayAudio.src);replayAudio.pause();}replayAudio=null;replayKey=null;if(key!=null)$('take-select').setRowPlaying(key,false);}
function replayTake(take,key){const stop=replayKey===key;stopReplay();if(stop)return;const url=URL.createObjectURL(wav(take.pcm));replayAudio=new Audio(url);replayKey=key;$('take-select').setRowPlaying(key,true);replayAudio.ontimeupdate=()=>{if(replayKey===key)$('take-select').setRowPlaying(key,true,replayAudio.currentTime/(replayAudio.duration||1));};replayAudio.onended=()=>{if(replayKey===key)replayKey=null;$('take-select').setRowPlaying(key,false);URL.revokeObjectURL(url);replayAudio=null;};replayAudio.play().catch(()=>{stopReplay();URL.revokeObjectURL(url);notify('再生できませんでした。',true);});}
$('take-select').addEventListener('optionrename',async e=>{
 const chosen=e.detail.key?takeChoices.find(t=>replayKeyOf(t)===e.detail.key):takeChoices[Number(e.detail.value)];if(!chosen)return;const name=e.detail.name;
 const id=chosen.takeId||chosen.storedId;
 if(!id){state.ownName=name;renderTakeMenu();await persistTakes();return;}
 try{const saved=await TakeStore.updateRecording(id,(snapshot,metadata)=>({snapshot,metadata:{...metadata,name}}));if(!saved)throw 0;state.takes=saved.index;if(state.ownTakeId===id){state.ownName=name;await persistTakes();}renderTakeMenu();}catch{renderTakeMenu();notify('名前を変更できませんでした。もう一度お試しください。',true);}
});
...
async function addJVS(files){if(!files.length||state.busy||state.recording)return;state.busy=true;controls();cancelAB();player.pause();reference.pause();const controller=importController=new AbortController();for(const id of ['choose-jvs-zip','choose-jvs-folder','import-audio'])$(id).disabled=true;$('cancel-jvs').hidden=false;$('jvs-progress').hidden=false;try{const result=await importJVS([...files],(done,total)=>{const pct=100*done/total;$('jvs-progress').setAttribute('aria-valuenow',String(Math.round(pct)));$('jvs-progress').firstElementChild.style.width=pct+'%';$('jvs-status').textContent=`${done.toLocaleString()} / ${total.toLocaleString()}`;},controller.signal);$('jvs-status').textContent=`${result.total.toLocaleString()}音声を追加済み`;}catch(e){$('jvs-status').textContent=e.name==='AbortError'?'中止しました。読み込み済みの音声は保存されています。':e.message;}finally{importController=null;state.busy=false;state.imported=await loadImported().catch(()=>state.imported);if(state.lang==='ja')await changeLanguage('ja',false);for(const id of ['choose-jvs-zip','choose-jvs-folder','import-audio'])$(id).disabled=false;$('cancel-jvs').hidden=true;$('jvs-progress').hidden=true;controls();$('jvs-zip').value='';$('jvs-folder').value='';}}
$('jvs-zip').onchange=e=>addJVS(e.target.files);$('jvs-folder').onchange=e=>addJVS(e.target.files);

let sessionReady=false;
function readView(){try{return JSON.parse(localStorage.getItem('koenami-session')||'null');}catch{return null;}}
function saveView(){if(!sessionReady)return;try{localStorage.setItem('koenami-session',JSON.stringify({lang:state.lang,group:$('library-group').value,sort:$('sort').value,search:$('search').value,reference:state.selected?.id,referenceRange:state.ranges.ref,openSpeakers:[...openSpeakers],dimension:map.dimension,projection:map.projection,yaw:map.yaw,tilt:map.tilt,zoom:map.zoom,camera:map.camera,center:map.center,pan:map.pan,autoRotate:map.autoRotate,signal:signal.mode,signalSource:signal.source,overlay:signal.overlay,liveShapeSeconds:map.liveShapeSeconds}));}catch{}}
setInterval(saveView,1000);window.addEventListener('beforeunload',saveView);document.addEventListener('visibilitychange',()=>{if(document.hidden){saveView();persistTakes();}});
