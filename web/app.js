...
import {Scorer,VERDICTS,LEANINGS,formatScore,ageText,gateFailure,verdictOf,representatives,distance2} from './score.js';
...
$('share-button').onclick=async()=>{
 ...
 const ageKey=`${state.ownTakeId||state.ownId||state.ownName}|${state.ownPCM?.length||0}|${state.ranges.own?state.ranges.own.map(v=>v.toFixed(2)).join('-'):'all'}`;$('share-age-include').checked=false;let result={...scored,age:undefined},bundle=shareBundle(result,scorer,lang),renders=0;
 ...
 const showAge=()=>{const a=ageEstimates.get(ageKey),lo=a&&Math.round(a.windowRange[0]),hi=a&&Math.round(a.windowRange[1]);$('share-age-value').textContent=a?`${ageText(a.estimate)}${a.windows>1&&lo!==hi?`（4秒ごとの推定 ${lo}〜${hi}）`:''}`:'';$('share-age-run').textContent=a?'もう一度推定':'推定する';$('share-age-include-label').hidden=!a;};showAge();
 const refresh=async()=>{const a=ageEstimates.get(ageKey);result={...scored,age:a&&$('share-age-include').checked?a.estimate:undefined};bundle=shareBundle(result,scorer,lang);applyBundle();shareImage=null;const token=++renders;try{const file=await render();if(token!==renders||!$('share-dialog').open)return;const img=$('share-image');if(img.src)URL.revokeObjectURL(img.src);img.src=URL.createObjectURL(file);img.alt=`${bundle.text}。5つの指標と見本の分布${result.age?'、年齢のめやす':''}を描いた画像。`;}catch(e){$('share-status').textContent=e.message;}};
 $('share-age-include').onchange=refresh;
 $('share-age-run').onclick=async()=>{if(!state.ownPCM)return;const r=state.ranges.own,pcm=r?state.ownPCM.slice(Math.round(r[0]*16000),Math.round(r[1]*16000)):state.ownPCM;$('share-age-run').disabled=true;$('share-age-run').textContent='推定中…';try{const a=await api('/api/age',{method:'POST',body:pcm});if(ageEstimates.size>=50)ageEstimates.delete(ageEstimates.keys().next().value);ageEstimates.set(ageKey,a);showAge();if($('share-age-include').checked)await refresh();}catch(e){$('share-status').textContent=e.message;}finally{$('share-age-run').disabled=false;$('share-age-run').textContent='推定する';}};
