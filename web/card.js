...
import {METRIC_KEYS,METRIC_LABELS,METRIC_UNITS,METRIC_DIGITS,VERDICTS} from './score.js';
...
function slider(x,y,w,result,bands){
 const pos=s=>x+w*clamp((s+10)/120,0,1);
 ...
 return `<rect x="${x}" y="${y-3}" width="${w}" height="6" rx="3" fill="${COLORS.grid}"/>${band('male',COLORS.sky)}${band('female',COLORS.pink)}<circle cx="${pos(result.score)}" cy="${y}" r="13" fill="${COLORS.self}" stroke="#fff" stroke-width="4"/><text x="${x}" y="${y+40}" font-size="20" fill="${COLORS.sky}">男性的</text><text x="${x+w}" y="${y+40}" font-size="20" fill="${COLORS.pink}" text-anchor="end">女性的</text>`;
}
