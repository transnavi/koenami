...
function eigen(matrix){
 const n=matrix.length,a=matrix.map(r=>[...r]),v=Array.from({length:n},(_,i)=>Array.from({length:n},(_,j)=>+(i===j)));
 for(let sweep=0;sweep<150;sweep++){
  let p=0,q=1;for(let i=0;i<n;i++)for(let j=i+1;j<n;j++)if(Math.abs(a[i][j])>Math.abs(a[p][q])){p=i;q=j;}
  ...
  for(let k=0;k<n;k++){if(k!==p&&k!==q){const kp=a[k][p],kq=a[k][q];a[k][p]=a[p][k]=c*kp-s*kq;a[k][q]=a[q][k]=s*kp+c*kq;}const vp=v[k][p],vq=v[k][q];v[k][p]=c*vp-s*vq;v[k][q]=s*vp+c*vq;}
 }
 const order=Array.from({length:n},(_,i)=>i).sort((i,j)=>a[j][j]-a[i][i]);
 return order.map(i=>{const axis=v.map(r=>r[i]),biggest=axis.reduce((b,x,k)=>Math.abs(x)>Math.abs(axis[b])?k:b,0);return axis.map(x=>x*(axis[biggest]<0?-1:1));});
}
function solve(matrix,b){
 const n=b.length,a=matrix.map((r,i)=>[...r,b[i]]);
 for(let c=0;c<n;c++){let p=c;for(let r=c+1;r<n;r++)if(Math.abs(a[r][c])>Math.abs(a[p][c]))p=r;[a[c],a[p]]=[a[p],a[c]];
  const d=a[c][c]||1e-12;for(let r=0;r<n;r++){if(r===c)continue;const f=a[r][c]/d;if(!f)continue;for(let k=c;k<=n;k++)a[r][k]-=f*a[c][k];}}
 return a.map((r,i)=>r[n]/(r[i]||1e-12));
}
function orthogonalize(vector,axes){const out=[...vector];for(const axis of axes){const d=out.reduce((s,x,k)=>s+x*axis[k],0);for(let k=0;k<out.length;k++)out[k]-=d*axis[k];}const norm=Math.sqrt(out.reduce((s,x)=>s+x*x,0));return norm>1e-6?out.map(x=>x/norm):null;}
export class AcousticSpace{
 ...
 constructor(samples){
  ...
  this.projections={variance:{axes:eigen(covariance)}};
  ...
  for(const p of Object.values(this.projections)){
   ...
   p.bounds=[0,1,2].map(k=>{const vals=projected.map(v=>v[k]);const low=quantile(vals,.01),high=quantile(vals,.99),pad=Math.max(.7,(high-low)*.28);return finite(low)?[low-pad,high+pad]:[-3,3];});
  }
 }
 standardized(f){const v=AcousticSpace.raw(f);return v.every(finite)?v.map((x,k)=>(x-this.center[k])/this.scale[k]):null;}
 projection(name){return this.projections[name]||this.projections.variance;}
 projectRaw(raw,axes){const z=raw.map((x,k)=>(x-this.center[k])/this.scale[k]-this.mean[k]);return axes.map(a=>a.reduce((s,v,k)=>s+v*z[k],0));}
 vector(f,name='variance'){const p=this.projection(name);const raw=AcousticSpace.raw(f);if(!raw.every(finite))return null;return this.projectRaw(raw,p.axes).slice(0,3).map((x,k)=>(x-p.bounds[k][0])/(p.bounds[k][1]-p.bounds[k][0]));}
 distance(a,b){const x=this.standardized(a),y=this.standardized(b);return x&&y?Math.sqrt(x.reduce((s,v,k)=>s+(v-y[k])**2,0)):Infinity;}
 comparison(a,b,dimensions=3,name='variance'){
  const p=this.projection(name),x=AcousticSpace.raw(a),y=AcousticSpace.raw(b);if(!x.every(finite)||!y.every(finite))return null;
  const left=this.projectRaw(x,p.axes),right=this.projectRaw(y,p.axes),squares=left.map((v,k)=>(v-right[k])**2),total=squares.reduce((sum,v)=>sum+v,0);
