...
export async function importJVS(files, progress, signal) {
  ...
  const ids = new Set(await TakeStore.read('jvs-index') || []);
  ...
  try {
    if (files.length === 1 && /\.zip$/i.test(files[0].name)) {
      ...
      for await (const entry of archive.getEntriesGenerator()) {
        ...
        if (clip && !ids.has(clip.id) && entry.uncompressedSize === clip.bytes)
          candidates.set(clip.id, { clip, read: () => entry.getData(new BlobWriter('audio/wav'), { signal }) });
      }
    } else {
    ...
    for (const { clip, read } of candidates.values()) {
      ...
      await TakeStore.writeEntries([[blob, 'jvs-audio:' + clip.id], [[...ids], 'jvs-index']]);
