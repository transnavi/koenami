...
for (const lang of LANGUAGES) {
	...
	for (const [key, value] of Object.entries(ja)) {
		...
		if (Array.isArray(value))
			value.forEach((item, i) => {
				const a = other[i],
					text = (x) => (typeof x === 'string' ? x : x.title + x.text);
				assert.equal(
					placeholders(text(a)),
					placeholders(text(item)),
					`${lang}: ${key}[${i}] placeholders`
				);
			});
		else {
			if (typeof other === 'object')
				assert.ok('other' in other, `${lang}: ${key} needs an "other" form`);
	...
	assert.ok(
		t('corpus.clips', { n: 1675 }).includes((1675).toLocaleString(lang)),
		`${lang}: number grouping`
	);
...
const index = readFileSync('web/index.html', 'utf8'),
	result = readFileSync('web/result.html', 'utf8');
for (const lang of LANGUAGES) {
	...
	assert.ok(
		page.includes(
			`<link rel="canonical" href="https://koe.transnavi.jp${lang === 'ja' ? '/' : `/${lang}/`}">`
		),
		`${lang}: canonical`
	);
	assert.ok(
		page.includes('<link rel="alternate" hreflang="x-default" href="https://koe.transnavi.jp/">'),
		`${lang}: x-default`
	);
	assert.ok(
		page.includes(`<title>${CATALOGUES[lang]['page.title'].replace(/&/g, '&amp;')}</title>`),
		`${lang}: title`
	);
	...
	const manifest = JSON.parse(
		renderManifest(readFileSync('web/public/site.webmanifest', 'utf8'), lang)
	);
...
assert.equal(
	home('ja'),
	'/',
	'the Japanese page (also served at /ja/) names the root as its address'
);
...
for (const lang of LANGUAGES) {
	const glyphs = codepoints(
		readFileSync(`web/public/fonts/koenami-share-${fontCut(lang)}-400.ttf`)
	);
	for (const verdict of ['female', 'androgynous', 'male']) {
		...
		const text = cardSVG(result, scorer, { lang })
		...
		assert.deepEqual(
			missing,
			[],
			`${lang}: characters missing from the ${fontCut(lang)} share font: ${missing.join('')}`
		);
	}
}
