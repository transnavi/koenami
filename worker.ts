...
async function resultPage(request: Request, env: Env, url: URL): Promise<Response> {
	...
	const title = t('result.window_title', {
	...
	const content: Record<string, string> = {
		...
		'og:image:alt': shareText(shared.result, lang)
	};
...
async function resultImage(
	...
	const renderer = new Resvg(svg, {
		font: {
			...
			defaultFontFamily: FONTS[shared.lang as keyof typeof FONTS]
		}
	});
	let png: Uint8Array;
	try {
...
async function handle(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	if (analysis) {
		...
		while (true) {
			...
			if (size > maxBytes) {
				await reader.cancel();
				return text(say(request, 'api.too_long'), 413);
			}
			chunks.push(value);
		}
