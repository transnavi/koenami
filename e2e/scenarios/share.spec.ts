...
test.describe('verdict and sharing', () => {
	test('readout, help dialogs, the share dialog, link, image and system share', async ({
		page,
		context,
		studio
	}) => {
		...
		await studio.tick(100);
		await studio.golden('link-copied', {
			extra: { clipboard: await page.evaluate(() => navigator.clipboard.readText()) }
		});
		await page.evaluate(() => {
		...
		await page.evaluate(() => {
			...
			Object.defineProperty(navigator, 'canShare', {
				value: (data: { files?: File[] }) => !!data.files,
				configurable: true
			});
			Object.defineProperty(navigator, 'share', {
				value: async (data: { title: string; text: string; url: string; files?: File[] }) => {
					w.__shared!.push({
						title: data.title,
						text: data.text,
						url: data.url,
						files: data.files?.map((f) => [f.name, f.type, f.size])
					});
				},
				configurable: true
			});
		...
		await studio.tick(100);
		await studio.golden('system-shared', {
			extra: { shared: await page.evaluate('window.__shared') }
		});
		...
		await studio.tick(100);
		await studio.golden('system-shared-without-file', {
			extra: { shared: await page.evaluate('window.__shared') }
		});
		await page.evaluate(() => {
			Object.defineProperty(navigator, 'canShare', { value: () => false, configurable: true });
		...
		await page.locator('#share-button').click();
		await studio.until(
			'document.getElementById("share-status").textContent === "画像を作成できませんでした。"'
		);
		await studio.tick(100);
	...
	test('the shared result page', async ({ page, context, studio }) => {
		...
		await studio.golden('result-link-copied', {
			extra: { clipboard: await page.evaluate(() => navigator.clipboard.readText()) }
		});
		await page.evaluate(() => {
			Object.defineProperty(navigator.clipboard, 'writeText', {
		...
		await studio.open(link.pathname + link.search, async (p) =>
			p.addInitScript(() => {
				Object.defineProperty(navigator, 'canShare', { value: () => true, configurable: true });
				Object.defineProperty(navigator, 'share', {
					value: async (data: { text: string; url: string; files?: File[] }) => {
						(window as unknown as { __shared: unknown[] }).__shared = [
							{ text: data.text, url: data.url, files: data.files?.length }
						];
					},
					configurable: true
				});
		...
		await studio.tick(100);
		await studio.golden('result-system-shared', {
			extra: { shared: await page.evaluate('window.__shared') }
		});
		await page.evaluate(() => {
			Object.defineProperty(navigator, 'share', {
		...
		for (const [name, query] of [
			...
			['midpoint', 'f0=163.54&df=1080.72&hnr=10.9&bal=-16.89&sp=5.86']
		] as const) {
	...
	test('age impression is opt-in and travels only when included', async ({ page, studio }) => {
		...
		await studio.tick(200);
		await studio.golden('age-optin-estimated', { extra: { link: await link() } });
		expect(await link()).not.toContain('age=');
		...
		await studio.tick(600);
		const image = await studio.download(() => page.locator('#share-save').click());
		await studio.golden('age-optin-included', { extra: { link: await link(), image } });

		await page.locator('#share-age-include').uncheck();
		await studio.until('!document.getElementById("share-open").href.includes("age=")');
		await studio.tick(600);
		await studio.golden('age-optin-excluded', { extra: { link: await link() } });
